...
import {
  ...
  GraduationCap, Award, Server, Circle,
} from "lucide-react";
...
function Reveal({ children, className = "", delay = 0 }) {
  ...
  return (
    <div
      ...
      className={`transform-gpu transition-all duration-700 ease-out ${shown ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"} ${className}`}
    >
      {children}
    </div>
  );
...
function Eyebrow({ index, children }) {
  return (
    <div className="mb-6 flex items-center gap-3 font-mono text-xs uppercase tracking-[0.2em] text-emerald-700">
...
function Token({ children }) {
  return (
    <span className="inline-flex items-center rounded-md border border-slate-200 bg-white px-2.5 py-1 font-mono text-xs text-slate-600">
      {children}
    </span>
  );
}
...
function Nav() {
  ...
  return (
    <header className={`sticky top-0 z-50 border-b transition-colors duration-300 ${scrolled ? "border-slate-200 bg-stone-50/85 backdrop-blur" : "border-transparent bg-transparent"}`}>
      <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-4">
        <a href="#top" className="group flex items-center gap-2.5">
          <span className="flex h-9 w-9 items-center justify-center rounded-md bg-slate-900 font-mono text-sm font-bold text-emerald-300">
            ny
          </span>
          <span className="font-mono text-sm font-medium tracking-tight text-slate-900">
            baig<span className="text-emerald-600">.dev</span>
          </span>
        </a>

        <nav className="hidden items-center gap-8 md:flex">
          {SECTIONS.map((s) => (
            <a key={s.id} href={`#${s.id}`} className="font-mono text-xs uppercase tracking-widest text-slate-500 transition-colors hover:text-emerald-700">
              {s.label}
            </a>
          ))}
          <a href={`mailto:${PROFILE.email}`} className="inline-flex items-center gap-1.5 rounded-md bg-slate-900 px-4 py-2 font-mono text-xs uppercase tracking-widest text-white transition-colors hover:bg-emerald-700">
            Hire me <ArrowUpRight className="h-3.5 w-3.5" />
          </a>
        </nav>

        <button onClick={() => setOpen((v) => !v)} className="md:hidden text-slate-900" aria-label="Toggle menu">
          {open ? <X className="h-6 w-6" /> : <Menu className="h-6 w-6" />}
        </button>
      </div>

      {open && (
        <div className="border-t border-slate-200 bg-stone-50 md:hidden">
          <nav className="mx-auto flex max-w-6xl flex-col px-6 py-3">
            {SECTIONS.map((s) => (
              <a key={s.id} href={`#${s.id}`} onClick={() => setOpen(false)} className="py-2.5 font-mono text-sm uppercase tracking-widest text-slate-600">
                {s.label}
              </a>
            ))}
            <a href={`mailto:${PROFILE.email}`} onClick={() => setOpen(false)} className="mt-2 inline-flex items-center justify-center gap-1.5 rounded-md bg-slate-900 px-4 py-2.5 font-mono text-xs uppercase tracking-widest text-white">
              Hire me <ArrowUpRight className="h-3.5 w-3.5" />
            </a>
          </nav>
        </div>
...
function Hero() {
  return (
    <section id="top" className="relative overflow-hidden">
      ...
      />
      <div className="pointer-events-none absolute -top-24 right-0 h-72 w-72 rounded-full bg-emerald-200 opacity-40 blur-3xl" />
      <div className="pointer-events-none absolute top-32 -left-20 h-72 w-72 rounded-full bg-amber-100 opacity-50 blur-3xl" />

      <div className="relative mx-auto max-w-6xl px-6 pb-20 pt-16 md:pb-28 md:pt-24">
        <div className="grid items-center gap-12 lg:grid-cols-[1.4fr_1fr]">
          <div>
            <Reveal>
              <div className="mb-8 inline-flex items-center gap-2 rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1 font-mono text-xs text-emerald-800">
            ...
            <Reveal delay={80}>
              <h1 className="font-display text-4xl font-semibold leading-[1.05] tracking-tight text-slate-900 sm:text-5xl md:text-6xl">
                I build production-grade{" "}
                <span className="text-emerald-700">React</span> applications
            ...
            <Reveal delay={160}>
              <p className="mt-6 max-w-xl text-lg leading-relaxed text-slate-600">
                ...
                web and mobile apps — from component systems and design tokens to
                GraphQL APIs and the NestJS services behind them.
              </p>
            </Reveal>

            <Reveal delay={240}>
              <div className="mt-9 flex flex-wrap items-center gap-3">
                <a href={`mailto:${PROFILE.email}`} className="inline-flex items-center gap-2 rounded-md bg-emerald-700 px-5 py-3 text-sm font-medium text-white shadow-sm transition-colors hover:bg-emerald-800">
                  <Mail className="h-4 w-4" /> Get in touch
                </a>
                <a href={PROFILE.github} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-md border border-slate-300 bg-white px-5 py-3 text-sm font-medium text-slate-800 transition-colors hover:border-slate-900">
                  <Github className="h-4 w-4" /> GitHub
                </a>
                <a href={PROFILE.linkedin} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-md border border-slate-300 bg-white px-5 py-3 text-sm font-medium text-slate-800 transition-colors hover:border-slate-900">
          ...
          <Reveal delay={200}>
            <div className="rounded-xl border border-slate-200 bg-white/70 p-1 shadow-sm backdrop-blur">
              ...
              <div className="rounded-lg bg-slate-900 p-5 font-mono text-sm">
...
function Experience() {
  return (
    <section id="work" className="mx-auto max-w-6xl px-6 py-20 md:py-28">
      ...
      <Reveal delay={60}>
        <h2 className="mb-14 max-w-2xl font-display text-3xl font-semibold tracking-tight text-slate-900 md:text-4xl">
      ...
      <div className="space-y-5">
        {EXPERIENCE.map((job, i) => (
          <Reveal key={i} delay={i * 60}>
            <article className="group relative grid gap-6 rounded-xl border border-slate-200 bg-white p-6 transition-all duration-300 hover:border-emerald-300 hover:shadow-md md:grid-cols-[200px_1fr] md:p-8">
              <div className="md:border-r md:border-slate-100 md:pr-6">
                <p className="font-mono text-xs uppercase tracking-widest text-emerald-700">{job.period}</p>
                <h3 className="mt-3 font-display text-lg font-semibold text-slate-900">{job.role}</h3>
                <p className="mt-1 text-sm text-slate-500">{job.company}</p>
              </div>

              <div>
                <p className="mb-4 inline-flex items-center gap-2 rounded-md bg-emerald-50 px-3 py-1 text-sm font-medium text-emerald-800">
          ...
          {SKILLS.map((group, i) => {
            ...
            return (
              <Reveal key={group.label} delay={i * 50}>
                <div className="h-full rounded-xl border border-slate-200 bg-stone-50 p-5 transition-colors hover:border-emerald-300">
                  <div className="mb-4 flex items-center gap-2.5">
                    <span className="flex h-8 w-8 items-center justify-center rounded-md bg-emerald-700 text-white">
                      <Icon className="h-4 w-4" />
                    </span>
                    <h3 className="font-mono text-xs uppercase tracking-widest text-slate-900">{group.label}</h3>
                  </div>
  ...
  return (
    <section id="education" className="mx-auto max-w-6xl px-6 py-20 md:py-28">
      ...
      <div className="grid gap-10 lg:grid-cols-2">
        <Reveal>
          <div>
            <h2 className="mb-7 flex items-center gap-2.5 font-display text-2xl font-semibold tracking-tight text-slate-900">
              <GraduationCap className="h-5 w-5 text-emerald-700" /> Education
            </h2>
            <div className="space-y-4">
              {EDUCATION.map((e) => (
                <div key={e.degree} className="rounded-xl border border-slate-200 bg-white p-5">
                  <p className="font-mono text-xs uppercase tracking-widest text-emerald-700">{e.period}</p>
                  <h3 className="mt-2 font-display text-base font-semibold text-slate-900">{e.degree}</h3>
                  <p className="mt-1 text-sm text-slate-500">{e.school}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <Reveal delay={100}>
          <div>
            <h2 className="mb-7 flex items-center gap-2.5 font-display text-2xl font-semibold tracking-tight text-slate-900">
              <Award className="h-5 w-5 text-emerald-700" /> Certifications
            </h2>
            <div className="space-y-3">
              {CERTS.map((c) => (
                <div key={c} className="flex items-start gap-3 rounded-xl border border-slate-200 bg-white p-5">
      ...
      <div className="relative mx-auto max-w-6xl px-6 py-24 md:py-32">
        ...
        <Reveal delay={180}>
          <div className="mt-10 grid gap-3 sm:grid-cols-2 lg:max-w-3xl lg:grid-cols-4">
            <a href={`mailto:${PROFILE.email}`} className="group flex flex-col gap-2 rounded-xl border border-slate-700 bg-slate-800/50 p-5 transition-colors hover:border-emerald-500">
            ...
            <a href={`tel:${PROFILE.phone.replace(/\s/g, "")}`} className="group flex flex-col gap-2 rounded-xl border border-slate-700 bg-slate-800/50 p-5 transition-colors hover:border-emerald-500">
            ...
            <a href={PROFILE.github} target="_blank" rel="noreferrer" className="group flex flex-col gap-2 rounded-xl border border-slate-700 bg-slate-800/50 p-5 transition-colors hover:border-emerald-500">
            ...
            <a href={PROFILE.linkedin} target="_blank" rel="noreferrer" className="group flex flex-col gap-2 rounded-xl border border-slate-700 bg-slate-800/50 p-5 transition-colors hover:border-emerald-500">
...
export default function App() {
  return (
    <div className="min-h-screen scroll-smooth bg-stone-50 font-sans text-slate-900 antialiased selection:bg-emerald-200 selection:text-emerald-900">
      <Nav />
      <main>
        <Hero />
        <Experience />
